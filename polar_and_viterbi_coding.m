...
%% Polar 
BER_sim = zeros(1,length(EbNodB));
BER_dB = zeros(1, length(EbNodB));
FER_sim = zeros(1,length(EbNodB));
Nbiterrs = zeros(1,length(EbNodB)); 
Nblkerrs = zeros(1,length(EbNodB)); 
EbNo = zeros(1, length(EbNodB));
Nblocks = 1000;

%% BPSK 
bpsk_msg = zeros(1,length(N));
de_bpsk_msg = zeros(1,length(N));
errorBPSK = zeros(1,length(EbNodB));
BPSK_Bits = zeros(1,length(EbNodB));
BER_raw = zeros(1,length(EbNodB));
BER_rawdB = zeros(1,length(EbNodB));

%% Viterbi 
constraintLength = 8;
feedBack = [247 255];
trel = poly2trellis(constraintLength, feedBack);

vite_bpsk_msg = zeros(1, length(N));
de_vite = zeros(1,length(N));
errorViterbi = zeros(1, length(EbNodB));
Vit_error = zeros(1, length(EbNodB));
BER_vite = zeros(1, length(EbNodB));
BER_vitedB = zeros(1,length(EbNodB));


%% 
for ii = 1:length(EbNodB) 

    EbNo(ii) = 10^(EbNodB(ii)/10);
    sigma = sqrt(1/(2*Rate*EbNo(ii)));
    ...
    for blk = 1:Nblocks
        ...
        bpsk_awgn = bpsk_msg + AWGN_noise(1:K);
        ...
        errorRaw = sum( msg ~= de_bpsk_msg);
        if errorRaw > 0
            errorBPSK(ii) = errorBPSK(ii) + errorRaw;
            BPSK_Bits(ii) = BPSK_Bits(ii) + 1;
        end

        %% Viterbi algorthm
        vite_msg = msg;
        vite_coded = convenc(vite_msg, trel);
        vite_cword = vite_coded;
        ...
        vite_awgn = vite_bpsk_msg + AWGN_noise;
        ...
        vite_decoder = vitdec(de_vite, trel, K, 'trunc', 'hard');
        errorVit = sum(msg ~= vite_decoder);

        if errorVit > 0
            errorViterbi(ii) = errorViterbi(ii) + errorVit;
            Vit_error(ii) = Vit_error(ii) + 1;
        end
            ...
            if depth == n
                ...
                if ns(npos) == 0    % step L and go to left child
                    ...
                    if ns(npos) == 1 % step R and go to right child
                        ...
                        node = node * 2 + 1; depth = depth + 1; % next node: left child
                        temp = temp / 2; % incoming belief length for left child
        ...
        if Nerrs > 0
            Nbiterrs(ii) = Nbiterrs(ii) + Nerrs;
            Nblkerrs(ii) = Nblkerrs(ii) + 1;
        end
        
    end
    
    % Polar BER
    BER_sim(ii) = Nbiterrs(ii)/(K*Nblocks);
    FER_sim(ii) = Nblkerrs(ii)/Nblocks;
    
    % BPSK BER
    BER_raw(ii) = errorBPSK(ii)/(K*Nblocks);
    
    % Viterbi BER
    BER_vite(ii) = errorViterbi(ii)/K/Nblocks;
  

    disp(ii)
    % disp([EbNodB(ii) FER_sim(ii) BER_sim(ii) Nblkerrs(ii) Nbiterrs(ii) Nblocks])


end

semilogy(EbNodB, BER_sim, "b-o",...
         EbNodB, BER_raw, 'r-o', ...
         EbNodB, BER_vite, 'g-s','LineWidth',2);
